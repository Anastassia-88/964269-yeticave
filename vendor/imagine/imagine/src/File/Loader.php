...
class Loader implements LoaderInterface
{
    ...
    public function __construct($path)
    {
        if (is_object($path) && !method_exists($path, '__toString')) {
            throw new InvalidArgumentException(sprintf('$path is an object of file %s which does not implement the __toString method',
                get_class($path)));
        }
    ...
    protected function readRemoteFileWithCurl()
    {
        ...
        if ($responseInfo['http_code'] < 200 || $responseInfo['http_code'] >= 300) {
            throw new InvalidArgumentException(sprintf('Failed to download "%s": %s', $this->path,
    ...
    protected function readRemoteFileWithFileGetContents()
    {
        ...
        $data = @file_get_contents($this->path);
        if ($data === false) {
            if (is_array($http_response_header) && isset($http_response_header[0]) && preg_match('/^HTTP\/\d+(?:\.\d+)*\s+(\d+\s+\w.*)/i',
                    $http_response_header[0], $matches)) {
